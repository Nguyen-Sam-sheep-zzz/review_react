...
function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    if (account.email === 'admin@gmail.com' && account.password === 'letmein') {
      setError('');
      // Chuyển sang trang danh sách Employee và đính kèm thông tin tài khoản đăng nhập
      navigate('/employee', { state: { userEmail: account.email } });
    } else {
    ...
    <div style={styles.container}>
      <form onSubmit={handleSubmit} style={styles.form}>
        ...
        <div style={styles.inputGroup}>
          <label>Email:</label>
          <input type="email" name="email" value={account.email} onChange={handleChange} required style={styles.input} />
        </div>
        <div style={styles.inputGroup}>
          <label>Password:</label>
          <input type="password" name="password" value={account.password} onChange={handleChange} required style={styles.input} />
        </div>
